document.addEventListener('DOMContentLoaded', function() {
    ...
    // clear button js
    const periodFilterResetButton = document.querySelector("#period-filter-reset-button");
    ...
    if (calculateButton) {
        calculateButton.onclick = function() {
            ...
            document.querySelectorAll(".transactions-table tbody tr").forEach(row => {
                ...
                const transactionAmount = parseFloat(row.children[2].textContent.replace(/[$,]/g,""));
                ...
                transactionType === "Income" ? totalIncome += transactionAmount : totalExpense += transactionAmount;
            });

            // update totals without refreshing to the hundredth decimal place
            document.getElementById("income-total-display").textContent = totalIncome.toFixed(2);
            document.getElementById("expense-total-display").textContent = totalExpense.toFixed(2);
            document.getElementById("net-total-display").textContent = (totalIncome - totalExpense).toFixed(2);
        }
    }
});
